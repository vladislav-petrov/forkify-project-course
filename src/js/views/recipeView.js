...
class RecipeView extends View {
                ...
                <div class="recipe__info">
                    ...
                    <div class="recipe__info-buttons">
                        <button
                            ...
                            data-servings="${this._data.servings - 1}"
                        >
                            <svg>
    ...
    subscribeHandlerRecipe(handler) {
        const events = ['load', 'hashchange'];
        events.forEach((event) => window.addEventListener(event, handler));
    }

    subscribeHandlerServings(handler) {
        this._parentElement.addEventListener('click', function(event) {
            const button = event.target.closest('.btn--tiny');

            if (!button || button.dataset.servings <= 0) return;

            handler(+button.dataset.servings);
        });
    }

    subscribeHandlerBookmark(handler) {
        this._parentElement.addEventListener('click', function(event) {
            const button = event.target.closest('.btn--round');
